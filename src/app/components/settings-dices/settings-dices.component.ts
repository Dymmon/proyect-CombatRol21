...
import { FigthersState, selectArray, selectMobs, selectMobsDices } from '../redux/interfaces/figthers.reducers';
import { PlayerInterface } from '../shared/player.interface';
import { SETMOBSDICES, SETPLAYERSSTATS } from '../redux/actions/fighters.actions';
...
export class SettingsDicesComponent implements OnInit {
  ...
  savePlayersInfo(event: any){
    this.store.dispatch(SETPLAYERSSTATS({payload: {array: this.playersUpdated}}));
    event.target.disabled = true;
  }
  saveMob(event: any){
    if(this.dicesForm.valid){
